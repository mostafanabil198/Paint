...
import eg.edu.alexu.csd.oop.draw.cs51_cs17.shapeHelpers.Helper;
...
public class DrawController implements DrawingEngine {
	...
	List<Class<? extends Shape>> returnedClasses = new ArrayList<Class<? extends Shape>>();
	...
	public void load(String path) {
		...
		try {
			if (path.toLowerCase().contains(".xml")) {
				xmlController.loadXml(shapes, path,returnedClasses);
				redoStack.clear();
			} else if (path.toLowerCase().contains(".json")) {
				json.loadJson(shapes, path,returnedClasses);
				redoStack.clear();
			}
			 undoStack.clear();
			 firstDraw = true;
	...
	public void installPluginShape(String jarPath) {
		...
		try {
			h.loadShapesFromJar(jarPath, returnedClasses);
